<%@ page import="com.bjpowernode.crm.setting.domain.DicValue" %>
...
<div style="position: relative; left: 40px; top: -50px;">
    ...
    <%
        ...
        if ("0".equals(currentPossibility)) {
            for (int i = 0; i < dvList.size(); i++) {
                ...
                String stageText = dvList.get(i).getText();
                String possibility = map.get(stage);
                if ("0".equals(possibility)) {
    ...
    <span id="<%=i%>" class="glyphicon glyphicon-remove mystage" onclick="changeStage('<%=stage%>','<%=i%>')"
          ...
          data-content="<%=stageText%>" style="color: #FF0000;"></span>
    -----------
    <%
    } else {
        //黑×
    ...
    <span id="<%=i%>" class="glyphicon glyphicon-remove mystage" onclick="changeStage('<%=stage%>','<%=i%>')"
          data-toggle="popover"
          data-placement="bottom"
          data-content="<%=stageText%>" style="color: #000000;"></span>
    -----------
    <%
        }
    } else {
    ...
    <span id="<%=i%>" class="glyphicon glyphicon-map-marker mystage" onclick="changeStage('<%=stage%>','<%=i%>')"
          ...
          data-content="<%=stageText%>" style="color: #90F790;"></span>
    -----------
    <%
    } else if (i < index) {
        //绿圈
